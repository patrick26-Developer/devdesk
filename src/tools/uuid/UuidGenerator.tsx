    ...
    >
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-xs text-muted-foreground">{t('ui.uuid.count')}</span>
        </div>

        {uuids.length > 1 && (
          <CopyButton value={uuids.join('\n')} label={t('common.copyAll')} variant="secondary" />
        )}
      </div>
      ...
      <div className="rounded-xl border border-border bg-muted/20 p-4">
        ...
        <p className="mt-1 text-[11px] leading-5 text-muted-foreground">{t('ui.uuid.note')}</p>
      </div>
    </ToolShell>
  );
}
