  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 rounded-lg border border-border bg-muted/30 px-3.5 py-2.5 text-xs text-muted-foreground">
        ...
        <span>
          Les empreintes sont calculées localement avec l'API Web Crypto native. Aucune donnée
      ...
      <Panel>
        <PanelHeader
          title="Texte source"
          subtitle="Le hash est recalculé automatiquement à chaque modification."
        />
        <Textarea
          value={input}
        ...
        {ALGORITHMS.map((algo) => {
          ...
          return (
            <Panel key={algo} className="min-h-0 hover:border-border">
              ...
              <PanelFooter>
                <span>{value ? `${value.length} car.` : 'En attente du texte source'}</span>
              </PanelFooter>
            </Panel>
          );
        })}
      ...
      <div className="flex items-center gap-2 border-t border-border pt-3 text-[10px] text-muted-foreground">
        ...
        <span>
          SHA-1 est conservé pour compatibilité. Pour les nouveaux usages de sécurité, privilégiez
