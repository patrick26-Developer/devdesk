...
export default function ContrastChecker() {
  ...
  const checks = ratio
    ? [
        { label: 'Texte normal · AA', pass: ratio >= 4.5, need: '4.5:1' },
        { label: 'Texte normal · AAA', pass: ratio >= 7, need: '7:1' },
        { label: 'Grand texte · AA', pass: ratio >= 3, need: '3:1' },
        { label: 'Grand texte · AAA', pass: ratio >= 4.5, need: '4.5:1' },
        { label: 'Éléments graphiques · AA', pass: ratio >= 3, need: '3:1' },
      ]
    : [];
  ...
  return (
    <ToolShell tool={tool}>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <ColorField label="Texte (premier plan)" value={fg} onChange={setFg} />
        <ColorField label="Fond (arrière-plan)" value={bg} onChange={setBg} />
      </div>
      ...
      <Panel>
        ...
        >
          <p className="text-lg font-semibold">Exemple de texte normal</p>
          <p className="text-sm">The quick brown fox jumps over the lazy dog — 0123456789</p>
        </div>
        <div className="flex items-center justify-between border-t border-border bg-card px-4 py-3">
          <span className="text-sm text-muted-foreground">Ratio de contraste</span>
      ...
      {ratio ? (
        ...
        <p className="rounded-lg border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs text-destructive">
          Saisissez deux couleurs hexadécimales valides (#rrggbb).
        </p>
      )}
    </ToolShell>
  );
