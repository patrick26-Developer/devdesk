...
export default function RequestBuilder({ request, onChange, onSend, onSave, loading, dirty }: RequestBuilderProps) {
  ...
  const tabs: { key: Tab; label: string; badge?: number }[] = [
    { key: 'params', label: 'Params', badge: count.params },
    { key: 'headers', label: 'Headers', badge: count.headers },
    { key: 'auth', label: 'Auth' },
    { key: 'body', label: 'Body' },
    { key: 'scripts', label: 'Pré-script' },
    { key: 'tests', label: 'Tests' },
    { key: 'extract', label: 'Extraction', badge: count.extract },
  ];
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex flex-col gap-2 border-b border-border p-3 md:flex-row">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="secondary" size="icon" onClick={onSave} className="h-10 w-10" title={dirty ? 'Enregistrer' : 'Enregistré'}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-auto p-4">
        {tab === 'params' && (
          <KvEditor rows={request.params} onChange={(params) => onChange({ params })} addLabel="Ajouter un paramètre" />
        )}
        {tab === 'headers' && (
          <KvEditor rows={request.headers} onChange={(headers) => onChange({ headers })} addLabel="Ajouter un en-tête" />
        )}
        ...
        {tab === 'scripts' && (
          <div className="flex min-h-0 flex-1 flex-col gap-2">
            <p className="text-xs text-muted-foreground">
              Exécuté avant l'envoi. API : <code className="font-mono">pm.variables.set()</code>,{' '}
              <code className="font-mono">pm.environment.set()</code>.
            </p>
            <Textarea
        ...
        {tab === 'tests' && (
          <div className="flex min-h-0 flex-1 flex-col gap-2">
            <p className="text-xs text-muted-foreground">
              Exécuté après la réponse. API : <code className="font-mono">pm.test(nom, fn)</code>,{' '}
              <code className="font-mono">pm.expect(x).to.equal(y)</code>,{' '}
              <code className="font-mono">pm.response.json()</code>, <code className="font-mono">pm.response.code</code>.
            </p>
            <Textarea
