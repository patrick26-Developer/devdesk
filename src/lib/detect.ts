...
export function detect(input: string): Detection[] {
  ...
  if ((text.startsWith('{') && text.endsWith('}')) || (text.startsWith('[') && text.endsWith(']'))) {
    try {
      JSON.parse(text);
      out.push({ toolId: 'json-formatter', label: 'detect.json', score: 0.95 });
      out.push({ toolId: 'json-to-ts', label: 'detect.jsonToTs', score: 0.5 });
      out.push({ toolId: 'json-yaml', label: 'detect.jsonToYaml', score: 0.4 });
    } catch {
      /* pas du JSON valide */
    }
  }
  ...
  if (JWT_RE.test(text) && text.split('.').length === 3) {
    out.push({ toolId: 'jwt', label: 'detect.jwt', score: 0.97 });
  }

  // UUID
  if (UUID_RE.test(text)) {
    out.push({ toolId: 'uuid', label: 'detect.uuid', score: 0.9 });
  }

  // Couleur hex
  if (HEX_COLOR_RE.test(text)) {
    out.push({ toolId: 'color', label: 'detect.hexColor', score: 0.9 });
    out.push({ toolId: 'contrast', label: 'detect.contrast', score: 0.4 });
  }
  ...
  if (/^\d{10}$|^\d{13}$/.test(text)) {
    ...
    if (n > 978307200000 && n < 4102444800000) {
      out.push({ toolId: 'timestamp', label: 'detect.timestamp', score: 0.85 });
    }
  }

  // URL
  if (/^https?:\/\/\S+$/i.test(text)) {
    out.push({ toolId: 'api-tester', label: 'detect.urlTest', score: 0.7 });
    out.push({ toolId: 'qrcode', label: 'detect.urlQr', score: 0.4 });
  }
  if (/%[0-9A-Fa-f]{2}/.test(text)) {
    out.push({ toolId: 'url', label: 'detect.urlEncoded', score: 0.6 });
  }

  // Cron
  if (CRON_RE.test(text) && /[*\d]/.test(text) && !text.includes('://')) {
    out.push({ toolId: 'cron', label: 'detect.cron', score: 0.6 });
  }
  ...
  if (text.includes('\n')) {
    ...
    if (lines.length >= 2 && commas[0] > 0 && commas.every((c) => c === commas[0])) {
      out.push({ toolId: 'csv-json', label: 'detect.csv', score: 0.7 });
    }
    if (lines.every((l) => /^[A-Z_][A-Z0-9_]*\s*=/i.test(l) || l.startsWith('#'))) {
      out.push({ toolId: 'dotenv-json', label: 'detect.dotenv', score: 0.75 });
    }
  }
  ...
  if (out.length === 0 && text.length >= 12 && text.length % 4 === 0 && BASE64_RE.test(text)) {
    out.push({ toolId: 'base64', label: 'detect.base64', score: 0.55 });
  }

  // Markdown
  if (out.length === 0 && /^#{1,6}\s|\*\*[^*]+\*\*|^[-*]\s|\[.+\]\(.+\)/m.test(text)) {
    out.push({ toolId: 'markdown', label: 'detect.markdown', score: 0.5 });
  }
